...
to setup-radius-range
  set radius-range (range -3 3)
end

to setup-cluster
  set cluster-position-x n-values init-cluster [random-pxcor]
  set cluster-position-y n-values init-cluster [random-pycor]
end
...
to setup-infected
  ...
  ask n-of num-infected turtles with [not vaccine?] [
   ...
   setxy ((item cluster cluster-position-x) + random-radius-x)  ((item cluster cluster-position-y) + random-radius-y)
   set color red
   set infected? true
  ]
end
...
to setup-vaccine
  let num-vaccine (init-vaccine / 100 * init-population)
...
to infect-susceptibles ;; S -> I
  ask turtles [
    let infected-neighbors (count other turtles with [color = red] in-radius (1 - quarantine.effort))
...
to-report %-max-infected
  report max-infected / init-population * 100
end
